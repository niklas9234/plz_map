...
const tradeStore = (() => {
    // Several independent UI components need the same, rarely changing master
    // data. Keep both completed and in-flight reads so rendering a company list
    // does not issue one request per trade badge.
    ...
    async function list(filters = {}) {
        ...
        const pending = request(path).catch((error) => {
            // A temporary failure must not remain cached: the next call should
            // be able to retry the API request.
            listCache.delete(path);
            throw error;
        });
        listCache.set(path, pending);
        return pending;
    }
