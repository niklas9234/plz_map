...
class Company(Base):
    ...
    created_at: Mapped[str] = mapped_column(String(35), nullable=False)
    updated_at: Mapped[str] = mapped_column(String(35), nullable=False)
    trades: Mapped[list["CompanyTrade"]] = relationship(cascade="all, delete-orphan")
    information: Mapped[list["CompanyInformation"]] = relationship(cascade="all, delete-orphan")


class CompanyTrade(Base):
    """A company's trade-specific configuration.

    Identity, status and contact information belong to the company; territories
    belong to this assignment so one company can serve different areas per trade.
    """

    __tablename__ = "company_trades"
...
class Territory(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["company_id", "trade_id"], ["company_trades.company_id", "company_trades.trade_id"],
